...
public class SerialTransport
{
    ...
    String comPort;
    public boolean connect() throws ReaderException
    {
        //comPort = deviceName;
        ...
        while(!isConnected)
        {
            try
            {
                ...
                int portIndex = 1;
                	...
                	try {
                		st = new SerialTransportNative(portName);
                        st.open();
                        incorrectPort = false;
    ...
    public void receiveStreaming() throws ReaderException
    {
        ...
        while (true) //Micah - no timer, have the program exit after gpio case
        {
            ...
            try
            {
                ...
                receiveMessage(25, m); //Micah changed timeout from 1000
                if (m.data[2] == 0x2F)
                {
                    //do nothing
                }
    ...
    {
        System.out.println("ßßß%"+ hexToAScii(t.toString()));
        if(t.data.length > 0)
        {
            System.out.println("Data: " + convertByteArrayToHexString(t.data));
        }
        JOptionPane.showMessageDialog(null,"ßßß%"+ hexToAScii(t.toString()));
        System.exit(0);
    }
...
//        }
        ...
        if (meta.contains(TagReadData.TagMetadataFlag.GPIO_STATUS)) //Micah - add print "You hit the button"
        {
...
//            }
    ...
    void parseResponse(Message m)
    {
        ...
        m.readIndex += 2;
        if (responseByte == 0x02) //Micah - This prints the tempature
        {
            m.readIndex += 2;
            while (m.readIndex < m.writeIndex)
            {
                ...
                if ((0 != statFlag) && (statFlag == ReaderStatsFlag.TEMPERATURE.value)) //Micah - Add Notify if temp > 80 inside this bracket, otherwise no
                {
                    TemperatureStatusReport tsr = new TemperatureStatusReport();
                    m.readIndex += 3;
                    tsr.temperature = m.getu8();

                    if (tsr.temperature >= 80) {
        ...
        else //Micah - this prints the message
        {
            ...
            {
                metadataFromMessage(t, m, metadataFlags); //Micah this grabs the gpio push
